		...
		while (index < _histogramsA.size()) {
			...
			if (_svm_type == svm_parameter.PRECOMPUTED) {
				...
				for (int j = 0; j < _histogramsB.size(); j++) {
					...
					else if (_svm_type_precomputed_kernel_type == AnomalyDetectionConfiguration.SVM_PRECOMPUTED_KERNEL_TYPE_RBF) {
						_retNode[index][j+1].value = this.gaussianKernel(oneHist, _histogramsB.get(j).getValue1());
					}
					else {
